...
import {
    ...
    ChevronRight,
    Settings,
    Unlock,            // ⟵ agregado para Liberaciones
...
/**
 * Lista base de elementos del sidebar.
 * show: usa tus flags existentes; para Liberaciones lo dejamos visible por defecto.
 */
const baseItems = computed(() => ([
    ...
    // ⟵ NUEVO: Liberaciones
    // Si luego agregas permisos, cambia 'show: true' por algo como: (ab.canViewLiberaciones || isAdmin.value)
    { label: 'Liberaciones',     icon: Unlock,          routeName: 'liberaciones.index', show: true },
    ...
    ...(isAdmin.value ? [{ label: 'Administración', icon: Settings, routeName: 'admin.index', show: true }] : []),
]))

/** Aplica filtro por texto del buscador y por flags 'show' */
        ...
        <div :class="[
            ...
            <nav class="mt-2 px-2 flex-1 overflow-y-auto">
                <ul class="space-y-1 py-1">
                    <li v-for="it in items" :key="it.routeName">
                        <Link
                            :href="route(it.routeName)"
                            :aria-current="isActive(it.routeName) ? 'page' : undefined"
                            class="group block"
                            Tooltip.bottom="!open ? it.label : null"
                        >
                            <div :class="[
            ...
            <div class="mt-auto px-3 pb-3">
                <div class="bg-gray-50 border border-gray-200 rounded-2xl p-2 flex items-center gap-3">
                    <button
                        class="w-full rounded-xl border border-gray-200 bg-white hover:bg-gray-50 text-gray-600 p-2"
                        @click="toggle"
