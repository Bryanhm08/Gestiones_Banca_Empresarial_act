...
import { BadgeDollarSign, Bookmark, Shield, CalendarDays, UserCircle, Save, ArrowLeft, Sparkles } from 'lucide-vue-next'
...
const props = defineProps({
    ...
    defaultAsesorId: { type: Number, default: null },
})
...
const form = ref({
    ...
    asesor_id: props.isAdmin ? props.defaultAsesorId : props.defaultAsesorId,
})
...
const invalid = computed(() => ({
    ...
    plazo: !(form.value.plazo >= 1),
    fecha_concesion: !form.value.fecha_concesion,
...
const submit = async () => {
    ...
    try {
        await axios.post(route('creditos.store'), {
            ...
            fecha_concesion: formatDate(form.value.fecha_concesion),
                    ...
                    <div class="flex items-start justify-between gap-6">
                        <div>
                            ...
                            <p class="mt-2 text-white/90 max-w-2xl">
                                Completá los datos del crédito. El vencimiento se calcula automáticamente con base en la
                                fecha y el plazo.
                            </p>
                            <div class="mt-4">
                ...
                <template #content>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div class="space-y-2">
                            ...
                            <Dropdown v-model="form.cliente_id" :options="clientes" optionLabel="nombre_cliente"
                                optionValue="id" filter placeholder="Seleccioná un cliente" class="w-full"
                        ...
                        <div class="space-y-2">
                            ...
                            <Dropdown v-model="form.tipo_credito_id" :options="tipos" optionLabel="nombre"
                                optionValue="id" placeholder="Seleccioná el tipo" class="w-full"
                            ...
                            <small v-else-if="errors.tipo_credito_id" class="text-red-500">{{ errors.tipo_credito_id[0]
                                }}</small>
                        </div>
                        <div class="space-y-2">
                            <label class="text-sm font-medium flex items-center gap-2">
                            ...
                            <Dropdown v-model="form.garantia_id" :options="garantias" optionLabel="nombre"
                                optionValue="id" placeholder="Seleccioná la garantía" class="w-full"
                            ...
                            <small v-else-if="errors.garantia_id" class="text-red-500">{{ errors.garantia_id[0]
                                }}</small>
                        </div>
                        <div class="space-y-2">
                            <label class="text-sm font-medium flex items-center gap-2">
                            ...
                            <InputNumber v-model="form.monto" mode="currency" currency="GTQ" locale="es-GT" :min="0"
                                :invalid="invalid.monto || !!errors.monto" class="w-full" />
                        ...
                        <div class="space-y-2">
                            ...
                            <InputNumber v-model="form.plazo" :min="1" :max="360" showButtons
                                :invalid="invalid.plazo || !!errors.plazo" class="w-full" />
                        ...
                        <div class="space-y-2">
                            ...
                            <Calendar v-model="form.fecha_concesion" dateFormat="yy-mm-dd" :showIcon="true"
                                showButtonBar class="w-full"
                                :invalid="invalid.fecha_concesion || !!errors.fecha_concesion" />
                            <small v-if="invalid.fecha_concesion" class="text-red-500">Seleccioná la fecha.</small>
                            <small v-else-if="errors.fecha_concesion" class="text-red-500">{{ errors.fecha_concesion[0]
                                }}</small>
                        </div>
                        <div v-if="isAdmin" class="space-y-2 md:col-span-2">
                            ...
                            <Dropdown v-model="form.asesor_id" :options="asesores" optionLabel="name" optionValue="id"
                                filter placeholder="Seleccioná el asesor" class="w-full"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <Card class="rounded-xl bg-gray-50">
                            ...
                            <template #content>
                                <div class="text-sm space-y-2">
                                    <p><span class="text-gray-500">Cliente:</span> {{(clientes.find(c =>
                                        c.id === form.cliente_id) || {}).nombre_cliente || '—' }}</p>
                                    <p><span class="text-gray-500">Tipo:</span> {{(tipos.find(t =>
                                        t.id === form.tipo_credito_id) || {}).nombre || '—' }}</p>
                                    <p><span class="text-gray-500">Garantía:</span> {{(garantias.find(g =>
                                        g.id ===form.garantia_id) || {}).nombre || '—' }}</p>
                                    <p><span class="text-gray-500">Monto:</span> {{ new
                                        Intl.NumberFormat('es-GT', { style: 'currency',currency:'GTQ'}).format(form.monto
                                        || 0) }}</p>
                                    <p><span class="text-gray-500">Plazo:</span> {{ form.plazo || '—' }} meses</p>
                                    <p><span class="text-gray-500">Concesión:</span> {{ formatDate(form.fecha_concesion)
                                        || '—' }}</p>
                                    <p><span class="text-gray-500">Vencimiento:</span> <strong>{{ fechaVencimientoUI
                                            }}</strong></p>
                                </div>
                            </template>
                        </Card>
                        ...
                        <div class="flex md:items-end justify-end gap-3">
                            ...
                            <Button :loading="loading" :disabled="loading" @click="submit"
                                v-tooltip.bottom="'Guardar crédito'">
