...
<template>
    <AuthenticatedLayout>
            ...
            <template #content>
                <DataTable :value="filtered" paginator :rows="10" :rowsPerPageOptions="[10, 20, 50]" dataKey="id"
                    ...
                    <Column header="Roles" style="width: 210px">
                        <template #body="{ data }">
                            <div class="flex items-center gap-2">
                                <Button size="small" outlined :severity="data.asesor ? undefined : 'secondary'"
                                    @click="toggleRoles(data, 'asesor')">
                                    Asesor: {{ data.asesor ? 'Sí' : 'No' }}
                                </Button>
                                <Button size="small" outlined :severity="data.admin ? 'success' : 'secondary'"
                    ...
                    <Column header="Estado" style="width: 120px">
                        <template #body="{ data }">
                            <Tag :class="data.estado ? 'bg-emerald-50 text-emerald-700 border-emerald-200'
        ...
        <!-- Modal Crear/Editar -->
        <Dialog v-model:visible="showForm" modal :style="{ width: '720px', maxWidth: '95vw' }"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <Dropdown v-model="form.area_id" :options="props.areas" optionLabel="nombre" optionValue="id"
                        class="w-full" placeholder="Seleccioná..." />
                </div>
                <div>
                    <label class="text-xs text-gray-500">Contraseña <span class="text-gray-400">(dejar en blanco para no
                            cambiar)</span></label>
                ...
                <div class="md:col-span-2">
                    <div class="flex flex-wrap gap-2">
                        <Button outlined :severity="form.asesor ? undefined : 'secondary'"
                            @click="form.asesor = !form.asesor">Asesor: {{ form.asesor ? 'Sí' : 'No' }}</Button>
                        <Button outlined :severity="form.admin ? 'success' : 'secondary'"
                            @click="form.admin = !form.admin">Admin: {{ form.admin ? 'Sí' : 'No' }}</Button>
                        <Button outlined :severity="form.estado ? 'success' : 'secondary'"
                            @click="form.estado = !form.estado">Activo: {{ form.estado ? 'Sí' : 'No' }}</Button>
                    </div>
                </div>
            </div>
