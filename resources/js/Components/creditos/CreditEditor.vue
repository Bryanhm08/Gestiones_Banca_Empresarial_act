...
import {
    ArrowLeft, Check, Plus, X, CalendarDays, BadgeDollarSign, Shield, User, Layers,
...
const save = async () => {
    try {
        saving.value = true
        await axios.patch(route('creditos.update', props.credito.id), {
        ...
        toast.add({ severity: 'success', summary: 'Guardado', detail: 'Crédito actualizado.', life: 2000 })
    } catch {
        toast.add({ severity: 'error', summary: 'Error', detail: 'No se pudo guardar.', life: 2400 })
    } finally {
        saving.value = false
    }
}
...
const stageMeta = (name = '') => {
    ...
    if (n.includes('análisis') || n.includes('analisis'))
        return { Icon: Search, chip: 'bg-sky-50 text-sky-700 border-sky-200', dot: 'bg-sky-500' }
    if (n.includes('comité') || n.includes('comite'))
        return { Icon: Users, chip: 'bg-violet-50 text-violet-700 border-violet-200', dot: 'bg-violet-500' }
    if (n.includes('formalización') || n.includes('formalizacion'))
        return { Icon: FileCheck, chip: 'bg-emerald-50 text-emerald-700 border-emerald-200', dot: 'bg-emerald-500' }
    if (n.includes('asignado desembolso'))
        return { Icon: Wallet, chip: 'bg-cyan-50 text-cyan-700 border-cyan-200', dot: 'bg-cyan-500' }
        ...
        <div class="flex items-start justify-between">
            ...
            <div class="flex gap-2">
                ...
                <Button :loading="saving" @click="save">
                    <Check class="w-4 h-4 mr-2" /> Guardar
                </Button>
            </div>
        </div>
                    ...
                    <div class="min-w-max pr-2">
                        <Timeline :value="estados" layout="horizontal" class="w-full">
                            <template #marker="slotProps">
                                <span
                                    class="flex items-center justify-center w-7 h-7 rounded-full ring-2 ring-white shadow"
                ...
                <template #content>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            ...
                            <InputNumber v-model="form.monto" mode="currency" currency="GTQ" locale="es-GT"
                                class="w-full" />
                        </div>
...
:deep(.p-timeline.p-timeline-horizontal .p-timeline-event-opposite) {
    margin-bottom: .25rem;
}

:deep(.p-timeline.p-timeline-horizontal .p-timeline-event-content) {
    margin-top: .25rem;
}
</style>
